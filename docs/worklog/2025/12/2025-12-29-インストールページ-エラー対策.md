...
## 追加修正: 「アプリケーションを初期化中...」から進まない問題の対策

### 問題

Shopify Adminからアプリにアクセスした際、「アプリケーションを初期化中...」のローディング画面から進まない問題が発生していました。

**原因**:
1. **`ZustandProvider`の初期化**: `isHydrated`が`false`のままで、`shouldSkipLoading`も`false`の場合、ローディング画面が表示され続ける
2. **`AuthProvider`の`isInitializing`が完了しない**: OAuth未完了時に`getToken()`がタイムアウトせず、`isInitializing`が`true`のままになる
3. **`vendors.css`のSyntaxError**: Next.jsのビルド時の問題で、アプリケーションの読み込みを妨げている可能性

### 修正内容

1. **`ZustandProvider`の初期化ロジック改善**:
   - `useState`の初期化時に`shouldSkipLoading`を評価しないように変更
   - `useEffect`で`pathname`が変更されたときに`shouldSkipLoading`を再評価し、スキップするページの場合は即座にハイドレーション完了とする

2. **`AuthProvider`の`getToken()`にタイムアウト処理を追加**:
   - OAuth未完了時に`getToken()`がタイムアウトせず、`isInitializing`が`true`のままになる問題を解決
   - 5秒のタイムアウトを設定し、タイムアウト時は`null`を返す

3. **`AuthProvider`の`isInitializing`タイムアウト処理を追加**:
   - 10秒以上`isInitializing`が`true`のままの場合、強制的に初期化を完了させる（無限ループ防止）
...
**効果**:
- OAuth未完了時でも、タイムアウト後に初期化が完了し、アプリケーションが進むようになる
- `ZustandProvider`のローディング画面が適切にスキップされる
- 無限ループが防止される

---

## 追加修正: ZustandProviderのskipLoadingPathsに/auth/exit-iframeを追加

### 問題

`ZustandProvider`の`skipLoadingPaths`に`/auth/exit-iframe`が含まれていないため、ExitIframeページにアクセスした際に「アプリケーションを初期化中...」のローディング画面が表示されたままになっていました。

### 修正内容

`ZustandProvider.tsx`の`skipLoadingPaths`に`/auth/exit-iframe`を追加しました。
...
**効果**:
- ExitIframeページにアクセスした際に、ローディング画面がスキップされ、即座にリダイレクト処理が実行される
- OAuth認証フローがスムーズに進行する
