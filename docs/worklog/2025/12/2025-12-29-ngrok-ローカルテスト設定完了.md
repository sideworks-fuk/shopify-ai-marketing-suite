# ngrok ローカルテスト設定完了

## 作業情報
- 開始日時: 2025-12-29
- 完了日時: 2025-12-29
- 担当: 福田＋AI Assistant

## 作業概要
ngrokを使用したローカル開発環境でのShopify OAuth認証テストのための設定を完了。

---

## ✅ 完了した設定項目

### 1. ngrokトンネルの起動 ✅

- **フロントエンド用ngrokトンネル**: 起動済み
- **URL**: `https://unsavagely-repressive-terrance.ngrok-free.dev`
- **ポート**: 3000（フロントエンド）
- **注意**: バックエンド用ngrokトンネルは不要（`localhost:7088`で直接接続）

### 2. フロントエンド環境変数の設定 ✅

**ファイル**: `frontend/.env.local`

設定内容：
```env
NEXT_PUBLIC_API_URL=https://localhost:7088
NEXT_PUBLIC_SHOPIFY_APP_URL=https://unsavagely-repressive-terrance.ngrok-free.dev
NEXT_PUBLIC_SHOPIFY_API_KEY=2d7e0e1f5da14eb9d299aa746738e44b
NEXT_PUBLIC_ENVIRONMENT=development
NEXT_PUBLIC_USE_HTTPS=true
NEXT_PUBLIC_DISABLE_FEATURE_GATES=true
```

### 3. `ShopifyApps`テーブルの`AppUrl`を更新 ✅

**更新内容**:
- **Id**: 6
- **AppUrl**: `https://unsavagely-repressive-terrance.ngrok-free.dev`
- **RedirectUri**: `https://unsavagely-repressive-terrance.ngrok-free.dev/api/shopify/callback`

**実行SQL**:
```sql
UPDATE [dbo].[ShopifyApps]
SET 
    [AppUrl] = 'https://unsavagely-repressive-terrance.ngrok-free.dev',
    [RedirectUri] = 'https://unsavagely-repressive-terrance.ngrok-free.dev/api/shopify/callback',
    [UpdatedAt] = GETUTCDATE()
WHERE [Id] = 6
  AND [ApiKey] = '706a757915dedce54806c0a179bee05d'
  AND [IsActive] = 1;
```

### 4. バックエンド設定（AllowedOrigins） ✅

**ファイル**: `backend/ShopifyAnalyticsApi/appsettings.Development.json`

**設定内容**:
```json
{
  "Cors": {
    "AllowedOrigins": [
      "http://localhost:3000",
      "https://unsavagely-repressive-terrance.ngrok-free.dev",
      "https://brave-sea-038f17a00-development.eastasia.1.azurestaticapps.net",
      "https://brave-sea-038f17a00-staging.eastasia.1.azurestaticapps.net",
      "https://brave-sea-038f17a00.1.azurestaticapps.net"
    ]
  }
}
```

**確認**: ✅ ngrok URLが追加済み

### 5. Shopify Partners Dashboardの設定 ✅

**設定内容**:
- **App URL**: `https://unsavagely-repressive-terrance.ngrok-free.dev`
- **Allowed redirection URL(s)**:
  1. `https://unsavagely-repressive-terrance.ngrok-free.dev/api/shopify/callback`
  2. `https://shopifyapp-backend-develop-a0e6fec4ath6fzaa.japanwest-01.azurewebsites.net/api/shopify/callback`（フォールバック用）
  3. `https://unsavagely-repressive-terrance.ngrok-free.dev/auth/success`（オプション）

---

### 6. バックエンド設定（UseFrontendProxy） ✅

**ファイル**: `backend/ShopifyAnalyticsApi/appsettings.LocalDevelopment.json`

**設定内容**:
```json
{
  "Shopify": {
    "UseFrontendProxy": true
  },
  "Cors": {
    "AllowedOrigins": [
      "https://unsavagely-repressive-terrance.ngrok-free.dev",
      ...
    ]
  }
}
```

**確認**: ✅ 設定済み

**注意**: 
- `launchSettings.json`で`ASPNETCORE_ENVIRONMENT=LocalDevelopment`が設定されているため、`appsettings.LocalDevelopment.json`が使用されます
- `Cors:AllowedOrigins`にも現在のngrok URLを追加済み
- **重要**: `Frontend:BaseUrl`は設定不要です。フロントエンドURLは`ShopifyApps`テーブルの`AppUrl`から自動的に取得されます

---

## ✅ すべての設定が完了しました

すべての設定項目が完了しました。次はOAuth認証フローのテストを実行してください。

---

## 🧪 テスト手順

### 1. バックエンドの起動

```powershell
# バックエンドを起動
cd backend/ShopifyAnalyticsApi
dotnet run
```

**注意**: 
- `launchSettings.json`で`ASPNETCORE_ENVIRONMENT=LocalDevelopment`が設定されているため、`appsettings.LocalDevelopment.json`が自動的に読み込まれます
- 環境変数の設定は不要です

### 2. フロントエンドの起動

```powershell
# 別のターミナルで実行
cd frontend
npm run dev
```

### 3. OAuth認証フローのテスト

1. ブラウザで `https://unsavagely-repressive-terrance.ngrok-free.dev/install?shop=your-dev-store.myshopify.com` にアクセス
2. 「接続を開始」ボタンをクリック
3. Shopify OAuth認証画面で権限を承認
4. フロントエンドの`/api/shopify/callback`にリダイレクトされることを確認
5. バックエンドの`/api/shopify/callback`にプロキシ転送されることを確認（ログで確認）
6. `/auth/success`ページにリダイレクトされることを確認
7. `/setup/initial`ページにリダイレクトされることを確認

---

## 📋 確認ポイント

### フロントエンドログ

**期待されるログ**:
```
🔐 [CallbackProxy] Shopify OAuthコールバック受信: { shop: 'xxx.myshopify.com', ... }
📤 [CallbackProxy] バックエンドへ転送: { url: 'https://localhost:7088/api/shopify/callback', ... }
📡 [CallbackProxy] バックエンド応答: { status: 302, location: '/auth/success?shop=xxx&storeId=xxx&success=true' }
↪️ [CallbackProxy] バックエンドからのリダイレクト: /auth/success?shop=xxx&storeId=xxx&success=true
```

### バックエンドログ

**期待されるログ**:
```
Redirect URI generated (frontend proxy): FrontendUrl=https://unsavagely-repressive-terrance.ngrok-free.dev, RedirectUri=https://unsavagely-repressive-terrance.ngrok-free.dev/api/shopify/callback
OAuth callback processing started. Shop: xxx.myshopify.com
Built embedded app URL (direct to /auth/success): https://unsavagely-repressive-terrance.ngrok-free.dev/auth/success?shop=xxx&storeId=xxx&success=true
```

### データベース確認

**Storesテーブルにレコードが作成されているか確認**:
```sql
SELECT TOP 10 
    [Id],
    [Domain],
    [ShopifyShopId],
    [AccessToken],
    [CreatedAt],
    [UpdatedAt]
FROM [dbo].[Stores]
ORDER BY [CreatedAt] DESC;
```

---

## 🔍 トラブルシューティング

### 問題1: バックエンドがフロントエンドプロキシを使用していない

**症状**: バックエンドログに「Redirect URI generated (backend direct)」が表示される

**原因**: 
- `appsettings.LocalDevelopment.json`の`Shopify:UseFrontendProxy`が`false`
- `ShopifyApps`テーブルの`AppUrl`が設定されていない、または`IsActive`が`false`

**解決方法**:
1. `appsettings.LocalDevelopment.json`に以下を追加：
```json
{
  "Shopify": {
    "UseFrontendProxy": true
  }
}
```

2. `ShopifyApps`テーブルの`AppUrl`を確認・更新：
```sql
UPDATE [dbo].[ShopifyApps]
SET 
    [AppUrl] = 'https://unsavagely-repressive-terrance.ngrok-free.dev',
    [RedirectUri] = 'https://unsavagely-repressive-terrance.ngrok-free.dev/api/shopify/callback',
    [UpdatedAt] = GETUTCDATE()
WHERE [Id] = 6
  AND [ApiKey] = '706a757915dedce54806c0a179bee05d'
  AND [IsActive] = 1;
```

**確認**: ✅ 既に設定済み

### 問題2: CORSエラーが発生する

**症状**: ブラウザのコンソールにCORSエラーが表示される

**原因**: `appsettings.Development.json`の`Cors:AllowedOrigins`にngrok URLが含まれていない

**解決方法**: ✅ 既に追加済み（確認済み）

### 問題3: Invalid callback URL エラー

**症状**: Shopify OAuth認証画面で「Invalid callback URL」エラーが表示される

**原因**: Shopify Partners DashboardのAllowed redirection URLsにngrok URLが登録されていない

**解決方法**: ✅ 既に設定済み（確認済み）

### 問題4: フロントエンドコールバックプロキシが動作しない

**症状**: `/api/shopify/callback`にアクセスしてもエラーになる

**原因**: `frontend/src/app/api/shopify/callback/route.ts`が存在しない、またはエラーが発生している

**解決方法**: 
- ファイルが存在することを確認
- フロントエンドのコンソールログを確認
- バックエンドへの接続が成功しているか確認

---

## ✅ 次のステップ

1. **バックエンドの起動**: `dotnet run`でバックエンドを起動
2. **OAuth認証フローのテスト**: 上記のテスト手順を実行
3. **ログ確認**: フロントエンドとバックエンドのログを確認
4. **データベース確認**: `Stores`テーブルにレコードが作成されているか確認

**注意**: すべての設定が完了しているため、バックエンドを起動してすぐにテストを開始できます。

---

## 📚 関連ドキュメント

- [ngrok-コールバックプロキシ実装](../05-development/01-環境構築/ngrok-コールバックプロキシ実装.md)
- [ngrok-ローカルテスト設定手順](../05-development/01-環境構築/ngrok-ローカルテスト設定手順.md)
- [ngrok-ローカルテスト-SQL更新](../05-development/01-環境構築/ngrok-ローカルテスト-SQL更新.md)
- [インストール機能設計書](../../03-feature-development/インストールフロー改善機能/インストール機能設計書.md)

---

## 📝 更新履歴

- 2025-12-29: 初版作成
  - 完了した設定項目を記録
  - 残りの設定項目を明確化
  - テスト手順と確認ポイントを追加
