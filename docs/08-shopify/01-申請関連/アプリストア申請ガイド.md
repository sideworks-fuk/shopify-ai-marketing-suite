...
**重要**: Shopifyの申請要件により、API連絡先メールアドレスに「Shopify」という単語を含めることはできません。

**現在の設定（エラー）:**
- `shopify@access-net.co.jp` ❌

**修正が必要:**
- Shopify Partners Dashboard > Settings > Contact information > API contact email
- 「Shopify」が含まれないメールアドレスに変更

**推奨メールアドレス:**
- `app@access-net.co.jp`
- `support@access-net.co.jp`
- `ec-ranger@access-net.co.jp`

**修正手順:**
1. Shopify Partners Dashboardにログイン
2. Apps > EC Ranger > Settings を選択
3. Contact information セクションを開く
4. API contact email の「Edit」ボタンをクリック
5. 新しいメールアドレスを入力して保存
...
### 4.4 Shopify Partners設定チェックリスト

- [ ] **API連絡先メールアドレスが「Shopify」を含まない** ⚠️
  - [ ] 現在の設定を確認: `shopify@access-net.co.jp` ❌
  - [ ] 修正が必要: 「Shopify」が含まれないメールアドレスに変更
  - [ ] 推奨: `app@access-net.co.jp` または `support@access-net.co.jp`
  - [ ] 修正場所: Partners Dashboard > Settings > Contact information
- [ ] OAuth設定（Redirect URLs）が正しい
