...
Shopifyアプリのアクセススコープ（権限）は、**Shopify Partners Dashboardで直接設定**しています。

**現在の設定方法**:
- Shopify Partners Dashboardでアプリのバージョンを作成し、スコープを選択
- バックエンドの`appsettings.json`でスコープを定義（OAuth認証時に使用）

**注意**: `shopify.app.toml`は現在使用していません（将来の使用予定）

---

## 📋 目次

1. [Shopify Partners Dashboardでの設定](#shopify-partners-dashboardでの設定) ⭐ **現在の設定方法**
2. [現在の実装でのスコープ取得方法](#現在の実装でのスコープ取得方法)
3. [shopify.app.tomlでの設定](#shopifyapptomlでの設定)（将来の使用予定）
4. [Phase 2（Shopify Managed Installation）でのスコープ管理](#phase-2shopify-managed-installationでのスコープ管理)（将来の使用予定）
...
```
read_all_orders,read_orders,read_products,read_customers
```

**詳細**:
- `read_all_orders`: すべての注文情報の読み取り（カスタムアプリ用）
  - 休眠顧客分析（最終購入日の取得）
  - 前年同月比分析（注文データの取得）
  - 購入回数分析（注文数の集計）
  - 月別売上統計（注文データの集計）
- `read_orders`: 注文情報の読み取り（フォールバック用）
  - `read_all_orders`が利用できない場合の代替
...
**注意**: 
- `read_all_orders`はカスタムアプリ専用のスコープです
- パブリックアプリの場合は`read_orders`のみが使用可能です
- 現在の実装では、カスタムアプリとして動作するため、`read_all_orders`を含む4つのスコープが設定されています

**確認結果**: この4つのスコープで現在の実装済み機能はすべて動作します。
...
## shopify.app.tomlでの設定（将来の使用予定）

**注意**: 現在は`shopify.app.toml`を使用していません。Shopify Partners Dashboardで直接設定しています。

### 将来の設定方法（参考）

将来的にShopify CLIを使用する場合は、`shopify.app.toml`ファイルに`scopes`を定義します：
...
3. デフォルト値: `"read_orders,read_products,read_customers"`（フォールバック用）

**注意**: 実際の設定ファイル（`appsettings.json`、`appsettings.Development.json`など）では、`read_all_orders,read_orders,read_products,read_customers`が設定されています。
...
## Phase 2（Shopify Managed Installation）でのスコープ管理（将来の使用予定）

**注意**: 現在はPhase 1の実装を使用しています。Phase 2への移行は将来の予定です。
...
**Phase 2（Shopify Managed Installation）**（将来の予定）:
...
- 2025-12-29: 最新のソースコードに合わせて修正
  - スコープを`read_all_orders,read_orders,read_products,read_customers`に更新
  - `read_all_orders`の説明を追加（カスタムアプリ専用スコープ）
  - 設定ファイルの実際の値を反映
  - `shopify.app.toml`は現在使用していないことを明記（将来の使用予定として記載）