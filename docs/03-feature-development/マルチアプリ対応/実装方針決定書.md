...
### 2.2 実装の容易さ
- ✅ **既存インフラを活用**: 追加のAzureリソース不要（Static Web Appsは2つ必要）
- ✅ **データベーススキーマ変更**: `ShopifyApps`テーブルを新規作成、`Store`テーブルに`ShopifyAppId`を追加
- ✅ **段階的な移行が可能**: 既存ストアはデフォルトCredentialsで動作（後方互換性）
...
**ShopifyAppsテーブルでアプリ情報を一元管理**

```sql
-- 初期データの投入（ShopifyAppsテーブル）
-- 公開アプリの登録
...
**フロントエンドからapiKeyを受け取り、ShopifyAppsテーブルから取得**

- フロントエンドから`apiKey`をクエリパラメータで受け取る（例: `/install?shop=store1.myshopify.com&apiKey=AAA`）
- バックエンドが`ShopifyApps`テーブルから該当アプリの情報を取得（apiKeyで検索）
- `ShopifyApp`が見つかれば、その`ApiKey`/`ApiSecret`/`Id`を使用
- 見つからない場合は、環境変数のデフォルト値を使用（フォールバック）
- OAuth認証完了後、`ShopifyAppId`を`Store`テーブルに保存
