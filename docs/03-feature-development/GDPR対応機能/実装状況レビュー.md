# GDPR対応機能 - 実装状況レビュー

**レビュー日**: 2025-12-31
**レビュー者**: 福田 + AI Assistant
**対象**: バックエンドソースコード

---

## 1. 実装済みファイル一覧

| ファイル | 概要 | 完了度 |
|----------|------|--------|
| `Controllers/WebhookController.cs` | Webhookエンドポイント | ✅ 100% |
| `Services/GDPRService.cs` | GDPR処理サービス | ✅ 100% |
| `Services/DataCleanupService.cs` | データ削除サービス | ✅ 100% |
| `Jobs/GdprProcessingJob.cs` | Hangfireジョブ | ✅ 100% |
| `Models/GDPRModels.cs` | エンティティモデル | ✅ 100% |
| `Middleware/HmacValidationMiddleware.cs` | HMAC検証 | ⚠️ 問題あり |

---

## 2. WebhookController.cs 詳細レビュー

### 2.1 エンドポイント実装状況

| エンドポイント | ルート | 実装状況 | 備考 |
|---------------|--------|---------|------|
| app/uninstalled | `POST /api/webhook/uninstalled` | ✅ 完了 | |
| customers/redact | `POST /api/webhook/customers-redact` | ✅ 完了 | |
| shop/redact | `POST /api/webhook/shop-redact` | ✅ 完了 | |
| customers/data_request | `POST /api/webhook/customers-data-request` | ✅ 完了 | |
| app_subscriptions/update | `POST /api/webhook/subscriptions-update` | ✅ 完了 | 課金連携 |
| app_subscriptions/cancel | `POST /api/webhook/subscriptions-cancel` | ✅ 完了 | 課金連携 |

### 2.2 実装されている機能

#### ✅ 必須ヘッダ検証
```csharp
// WebhookController.cs:936-970
private bool ValidateRequiredHeaders(string expectedTopic, out string topic, out string shopDomain, out string webhookId)
{
    topic = Request.Headers["X-Shopify-Topic"].ToString();
    shopDomain = Request.Headers["X-Shopify-Shop-Domain"].ToString();
    webhookId = Request.Headers["X-Shopify-Webhook-Id"].ToString();
    
    // 許可トピックリストで検証
    var allowed = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        "app/uninstalled", "customers/redact", "shop/redact",
        "customers/data_request", "app_subscriptions/update", "app_subscriptions/cancel"
    };
    // ...
}
```

#### ✅ HMAC検証（マルチアプリ対応）
```csharp
// WebhookController.cs:429-511
private async Task<bool> VerifyWebhookRequest()
{
    // 1. X-Shopify-Hmac-SHA256ヘッダー取得
    // 2. X-Shopify-Shop-DomainからストアのShopifyAppを特定
    // 3. ShopifyApp.ApiSecretでHMAC検証
    // 4. フォールバック: 設定ファイルのWebhookSecret
    // 5. 固定時間比較で検証
    return CryptographicOperations.FixedTimeEquals(computedHashBytes, receivedHmacBytes);
}
```

#### ✅ 即時200応答（5秒ルール対策）
```csharp
// WebhookController.cs:87-107
// 非同期で処理を実行
_ = Task.Run(async () =>
{
    try
    {
        await CancelStoreSubscription(webhook.Domain);
        await ScheduleDataDeletion(webhook.Domain, 48);
    }
    catch (Exception ex) { /* ログ記録 */ }
});

// 即座に200 OKを返す（5秒ルール）
return Ok();
```

#### ✅ 冪等性キー生成
```csharp
// WebhookController.cs:977-1001
private string GenerateWebhookIdempotencyKey(string shopDomain, string topic, string payloadJson)
{
    // hash(StoreDomain + Topic + created_at + X-Shopify-Webhook-Id)
    var seed = $"{shopDomain}|{topic}|{createdAt}|{webhookId}";
    using var sha = SHA256.Create();
    var hash = sha.ComputeHash(Encoding.UTF8.GetBytes(seed));
    return Convert.ToBase64String(hash);
}
```

#### ✅ サイズ上限チェック
```csharp
// WebhookController.cs:24
[RequestSizeLimit(262144)] // 256 KB 上限

// WebhookController.cs:522-527
if (Encoding.UTF8.GetByteCount(body) > 262144)
{
    throw new InvalidOperationException("Payload too large");
}
```

### 2.3 問題点・課題

#### 🔴 本番削除スケジューリングが未完成

```csharp
// WebhookController.cs:616-626
if (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development" || daysToDelete == 0)
{
    // 開発環境では即座に削除
    await _dataCleanupService.DeleteStoreDataAsync(shopDomain);
}
else
{
    // TODO: 本番環境ではHangfireでスケジュール
    _logger.LogInformation("本番環境では{Days}日後にデータを削除します...");
}
```

**同様のTODOが以下にも存在**:
- `ScheduleDataDeletion()` - 624行目
- `ScheduleCustomerDataDeletion()` - 649行目
- `ScheduleShopDataDeletion()` - 659行目
- `ScheduleCustomerDataExport()` - 672行目

---

## 3. GDPRService.cs 詳細レビュー

### 3.1 実装されている機能

| メソッド | 目的 | 状況 |
|---------|------|------|
| `CreateRequestAsync()` | GDPR要求を作成 | ✅ 完了 |
| `ProcessCustomerDataRequestAsync()` | データエクスポート | ✅ 完了 |
| `ProcessCustomerRedactAsync()` | 顧客データ削除 | ✅ 完了 |
| `ProcessShopRedactAsync()` | ショップデータ削除 | ✅ 完了 |
| `MarkAsCompletedAsync()` | 完了マーク | ✅ 完了 |
| `MarkAsFailedAsync()` | 失敗マーク（リトライ対応） | ✅ 完了 |
| `GetStatisticsAsync()` | 統計取得 | ✅ 完了 |
| `GenerateComplianceReportAsync()` | レポート生成 | ✅ 完了 |

### 3.2 期限計算ロジック

```csharp
// GDPRService.cs:513-521
private DateTime CalculateDueDate(string requestType)
{
    return requestType.ToLower() switch
    {
        "customers_data_request" => DateTime.UtcNow.AddDays(10),  // 10日以内
        "customers_redact" => DateTime.UtcNow.AddDays(30),        // 30日以内
        "shop_redact" => DateTime.UtcNow.AddDays(90),            // 90日以内
        _ => DateTime.UtcNow.AddDays(30)
    };
}
```

### 3.3 リトライ・バックオフ

```csharp
// GDPRService.cs:406-414
if (request.CanRetry)
{
    request.Status = "pending";
    request.ScheduledFor = DateTime.UtcNow.AddHours(Math.Pow(2, request.RetryCount)); // 指数バックオフ
}
```

---

## 4. GdprProcessingJob.cs 詳細レビュー

### 4.1 Hangfireジョブ登録

```csharp
// Program.cs:311-323
RecurringJob.AddOrUpdate<GdprProcessingJob>(
    "gdpr-process-pending",
    job => job.ProcessPendingRequests(),
    "*/5 * * * *");  // 5分ごとに実行
```

### 4.2 処理ロジック

```csharp
// GdprProcessingJob.cs:24-66
[DisableConcurrentExecution(timeoutInSeconds: 300)]
public async Task ProcessPendingRequests()
{
    var pending = await _gdprService.GetPendingRequestsAsync();
    foreach (var request in pending)
    {
        switch (request.RequestType.ToLower())
        {
            case "customers_data_request":
                await _gdprService.ProcessCustomerDataRequestAsync(request.Id);
                break;
            // ...
        }
    }
}
```

---

## 5. HmacValidationMiddleware.cs 問題分析

### 5.1 二重検証問題

現在、HMAC検証が**2箇所で実行**されています：

1. **ミドルウェア** (`HmacValidationMiddleware.cs`)
   - `Program.cs:542` で登録
   - `/api/webhook/*` パスに適用
   - **設定ファイルの`WebhookSecret`のみ使用**
   - 失敗時: **401 Unauthorized**

2. **コントローラー** (`WebhookController.cs:VerifyWebhookRequest()`)
   - 各エンドポイントで個別に呼び出し
   - **マルチアプリ対応（ストア別ApiSecret取得）**
   - 失敗時: **403 Forbid**

### 5.2 問題の影響

```
Shopify → Webhookを送信
    ↓
HmacValidationMiddleware (設定ファイルのWebhookSecretで検証)
    ↓ 失敗 → 401 Unauthorized ← 問題発生箇所
    ↓ 成功
WebhookController.VerifyWebhookRequest() (ストア別ApiSecretで検証)
    ↓ 失敗 → 403 Forbid
    ↓ 成功
処理実行
```

### 5.3 設定ファイルの現状

```json
// appsettings.Development.json
"WebhookSecret": ""  // 空文字

// appsettings.Staging.json
"WebhookSecret": "your_staging_webhook_secret"  // プレースホルダー

// appsettings.Production.json
"WebhookSecret": ""  // 空文字または未設定
```

**問題**: `WebhookSecret`が未設定または無効な値のため、ミドルウェアで401エラーが発生

### 5.4 修正方針

**推奨**: ミドルウェアを無効化し、コントローラーのマルチアプリ対応検証のみを使用

```csharp
// Program.cs - 以下をコメントアウト
// app.UseHmacValidation();
```

---

## 6. GDPRModels.cs 詳細レビュー

### 6.1 エンティティ一覧

| エンティティ | 目的 | 状況 |
|-------------|------|------|
| `GDPRRequest` | GDPR要求管理 | ✅ 完了 |
| `GDPRDeletionLog` | 削除監査証跡 | ✅ 完了 |
| `GDPRStatistics` | 統計データ | ✅ 完了 |

### 6.2 計算プロパティ

```csharp
// GDPRModels.cs:151-158
[NotMapped]
public bool IsOverdue => Status != "completed" && DateTime.UtcNow > DueDate;

[NotMapped]
public int DaysUntilDue => (int)(DueDate - DateTime.UtcNow).TotalDays;

[NotMapped]
public bool CanRetry => RetryCount < MaxRetries && Status == "failed";
```

---

## 7. 総合評価

### 7.1 完了度サマリー

| カテゴリ | 完了度 | 詳細 |
|---------|--------|------|
| Webhookエンドポイント | ✅ 100% | 6種すべて実装済み |
| HMAC検証（コントローラー） | ✅ 100% | マルチアプリ対応済み |
| 即時200応答 | ✅ 100% | 非同期処理パターン採用 |
| 冪等化処理 | ✅ 100% | IdempotencyKey実装済み |
| GDPR要求管理 | ✅ 100% | 作成・更新・完了管理 |
| データ削除サービス | ✅ 100% | 顧客・ショップ削除対応 |
| 監査ログ | ✅ 100% | 削除証跡の記録 |
| Hangfire定期ジョブ | ✅ 100% | 5分間隔で実行 |
| **本番削除スケジュール** | 🔴 0% | TODOのまま |
| **ミドルウェア二重検証** | 🔴 要修正 | 401エラーの原因 |

### 7.2 コード品質

- ✅ ログ出力が適切（個人情報を含まない）
- ✅ 例外処理が実装済み
- ✅ 非同期処理パターンを適切に使用
- ✅ 固定時間比較によるタイミング攻撃対策
- ⚠️ DBユニーク制約が未確認（コード側でのみ重複チェック）

---

## 8. 次のアクション

1. **ミドルウェア無効化**（即時対応）
2. **本番スケジューリング実装**（0.5日）
3. **DBユニーク制約追加**（0.25日）
4. **E2E検証**（0.5日）
