...
BEGIN
    ...
    VALUES 
        ('EC Ranger', 'EC Ranger - 公開アプリ（開発環境）', 'Public', 
         'be1fc09e2135be7cee3b9186ef8bfe80', -- Client ID（Shopify Partners Dashboardの「Client ID」）
         '[YOUR_DEVELOPMENT_PUBLIC_APP_API_SECRET]', -- Secret（Shopify Partners Dashboardの「Secret」）⚠️ 実際の値に置き換えてください
         'https://localhost:3000', -- 開発環境のフロントエンドURL
         'https://localhost:3000/api/shopify/callback',
         'read_orders,read_products,read_customers',
         1, GETUTCDATE(), GETUTCDATE());

    PRINT '開発環境用の公開アプリを登録しました。';
END
ELSE
BEGIN
...
BEGIN
    INSERT INTO [dbo].[ShopifyApps] 
        ([Name], [DisplayName], [AppType], [ApiKey], [ApiSecret], [AppUrl], [RedirectUri], [Scopes], [IsActive], [CreatedAt], [UpdatedAt])
    VALUES 
        ('EC Ranger Demo', 'EC Ranger - カスタムアプリ（開発環境）', 'Custom', 
         '[YOUR_DEVELOPMENT_CUSTOM_APP_API_KEY]', -- Client ID（Shopify Partners Dashboardの「Client ID」）⚠️ 実際の値に置き換えてください
         '[YOUR_DEVELOPMENT_CUSTOM_APP_API_SECRET]', -- Secret（Shopify Partners Dashboardの「Secret」）⚠️ 実際の値に置き換えてください
         'https://localhost:3001', -- 開発環境のカスタムアプリURL
         'https://localhost:3001/api/shopify/callback',
         'read_orders,read_products,read_customers',
         1, GETUTCDATE(), GETUTCDATE());

    PRINT '開発環境用のカスタムアプリを登録しました。';
END
ELSE
BEGIN
