-- ============================================================
-- データ同期テスト前のデータクリアスクリプト（v2）
-- ============================================================
-- 用途: データ同期機能の再テスト前に、既存の同期データをクリアする
...
-- 1. パラメータ設定
-- ============================================================
DECLARE @StoreId INT = 46; -- テスト対象のStoreIdを指定（開発環境テスト用）
...
-- OrderItems（Ordersに紐付く）
DECLARE @OrderItemCount INT;
SELECT @OrderItemCount = COUNT(*)
FROM OrderItems oi
JOIN Orders o ON oi.OrderId = o.Id
WHERE o.StoreId = @StoreId;
PRINT '注文明細データ: ' + CAST(@OrderItemCount AS NVARCHAR(10)) + ' 件';
...
BEGIN
    ...
    JOIN Orders o ON oi.OrderId = o.Id
    WHERE o.StoreId = @StoreId;
    PRINT '注文明細データ削除完了: ' + CAST(@@ROWCOUNT AS NVARCHAR(10)) + ' 件';
